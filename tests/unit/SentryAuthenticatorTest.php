...
class SentryAuthenticatorTest extends DbTestCase
{
    ...
    /**
     * @test
     * @expectedException \Jai\Authentication\Exceptions\AuthenticationErrorException
     **/
    ...
    /**
     * @test
     * @expectedException \Jai\Authentication\Exceptions\UserNotFoundException
     **/
    ...
    public function itCanGetActivationToken()
    {
        $mock_user = m::mock('StdClass')
                      ...
                      ->andReturn(true)
                      ->getMock();
        $mock_auth = m::mock('Jai\Authentication\Classes\SentryAuthenticator')
                      ->makePartial()
    ...
    {
        $authenticator = m::mock('Jai\Authentication\Classes\SentryAuthenticator')
                          ->makePartial()
                          ->shouldReceive('getUser')
                          ->once()
                          ->with($email)
    ...
    {
        $unbanned_user = $this->make('Jai\Authentication\Models\User', array_merge($this->getUserStub(), ["banned" => 0]))->first();
