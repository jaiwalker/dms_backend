...
class UserControllerTest extends DbTestCase
{
    ...
    public function it_run_signup_and_return_success_on_post_signup()
    {
        ...
        $this->action('POST', 'Jai\Authentication\Controllers\UserController@postSignup');

        $this->assertRedirectedToAction('Jai\Authentication\Controllers\UserController@signupSuccess');
    ...
    public function it_run_signup_and_return_errors_on_post_signup()
    {
        ...
        $this->assertRedirectedToAction('Jai\Authentication\Controllers\UserController@signup');
    ...
    {
        $this->action('GET', 'Jai\Authentication\Controllers\UserController@signup');
    ...
    public function itDoesntShowCaptchaOnSignupIfDisabled()
    {
        ...
        $response = $this->action('GET', 'Jai\Authentication\Controllers\UserController@signup');
    ...
    public function it_showConfirmationEmailSuccessOnSignup_ifEmailConfirmationIsEnabled()
    {
        ...
        $this->action('GET', 'Jai\Authentication\Controllers\UserController@signupSuccess');
    }
    ...
    public function it_show_view_with_success_if_token_is_valid()
    {
        ...
        $this->action('GET', 'Jai\Authentication\Controllers\UserController@emailConfirmation',
            '', [
                "email" => $email,
                "token" => $token
            ]);
    ...
    public function it_show_view_with_error_if_token_is_invalid()
    {
        ...
        $token = "_token";
        $mock_service = m::mock('StdClass')->shouldReceive('checkUserActivationCode')->once()->with($email,
            $token)->andThrow(new \Jai\Authentication\Exceptions\TokenMismatchException)->shouldReceive('getErrors')->once()->andReturn("")->getMock();
        App::instance('register_service', $mock_service);
    ...
    public function it_show_view_errors_if_user_is_not_found()
    {
        ...
        $token = "_token";
        $mock_service = m::mock('StdClass')->shouldReceive('checkUserActivationCode')->once()->with($email,
            $token)->andThrow(new \Jai\Authentication\Exceptions\UserNotFoundException())->shouldReceive('getErrors')->once()->andReturn("")->getMock();
        App::instance('register_service', $mock_service);
    ...
    public function it_show_user_lists_on_lists()
    {
        ...
        $this->action('GET', 'Jai\Authentication\Controllers\UserController@getList', [
            "new" => "new input",
    ...
    public function createNewUserWithSuccess()
    {
        ...
        $this->action('POST', 'Jai\Authentication\Controllers\UserController@postEditUser', $input_data);
        ...
        $this->assertRedirectedToAction('Jai\Authentication\Controllers\UserController@editUser',
            ['id' => $user_created->id]);
    ...
    {
        $user_created = $this->make('Jai\Authentication\Models\User', $this->getUserStub());
    ...
    public function canAddCustomFieldType()
    {
        ...
        $user_id = 1;
        $this->action('POST', 'Jai\Authentication\Controllers\UserController@addCustomFieldType', ['description' => $field_description, 'user_id' => $user_id]);
        ...
        $this->assertRedirectedToAction('Jai\Authentication\Controllers\UserController@postEditProfile', ["user_id" => $user_id]);
        $this->assertSessionHas('message');
    }
    ...
    {
        $user_created = $this->make('Jai\Authentication\Models\User', array_merge($this->getUserStub(),["permissions" => ["_perm" => 1]]));
    ...
    public function canDeleteCustomFieldType()
    {
        ...
        $this->action('POST', 'Jai\Authentication\Controllers\UserController@deleteCustomFieldType', ["id" => $field_id, "user_id" => $user_id]);
    ...
    {
        $created_user = $this->make('Jai\Authentication\Models\User', $this->getUserStub())->first();
        $created_user_profile = $this->make('Jai\Authentication\Models\UserProfile', $this->getUserProfileStub($created_user))->first();
    ...
    {
        $mock_logged_user = m::mock('Jai\Authentication\Interfaces\AuthenticateInterface')->shouldReceive('getLoggedUser')
                             ->once()
                             ->andReturn($created_user)
                             ->getMock();
        App::instance('Jai\Authentication\Interfaces\AuthenticateInterface', $mock_logged_user);
    }
}
