...
class UserRegisterServiceTest extends DbTestCase
{
    ...
    {
        $mock_validator = m::mock('Jai\Authentication\Validators\UserSignupValidator')->shouldReceive('validate')->andReturn(true)->getMock();
    ...
    {
        return m::mock('Jai\Authentication\Validators\UserSignupValidator')
    ...
    /**
     * @test
     * @expectedException \Jai\Authentication\Exceptions\UserExistsException
     **/
    ...
    {
        return m::mock('Jai\Authentication\Interfaces\AuthenticateInterface')
    ...
    public function setupActiveStateOfUser()
    {
        $this->disableEmailConfirmation();
        Config::set('laravel-authentication-acl::email_confirmation',false);
        $service = m::mock('Jai\Authentication\Services\UserRegisterService');
    ...
    public function checksForActivationAndThrowsUserNotFoundException()
    {
        ...
        {
            $service->checkUserActivationCode($email, $token);
        } catch(\Jai\Authentication\Exceptions\UserNotFoundException $e)
        {
            $gotcha = true;
        }
    ...
    {
        $user_stub =
                m::mock('Jai\Authentication\Modles\User')->makePartial()->shouldReceive('checkResetPasswordCode')->andReturn(false)->getMock();
        ...
        {
            $service->checkUserActivationCode($email, $token);
        } catch(\Jai\Authentication\Exceptions\TokenMismatchException $e)
        {
            $gotcha = true;
        }
    ...
    {
        $user_stub = m::mock('Jai\Authentication\Modles\User');
        $user_stub->activation_code = "12345_";
